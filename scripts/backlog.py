...
def load_chantier(dirpath: Path, state: str) -> Chantier:
    ...
    ch.docs = sorted(p.name for p in dirpath.glob("*.html"))
    return ch
...
def fiche_rows(fiches: list[Fiche]) -> str:
    rows = []
    for f in sorted(fiches, key=lambda x: (x.priority or "P9", x.id)):
        dep = ", ".join(f"<code>{esc(d)}</code>" for d in f.depends_on if d)
        rows.append(
            f"<tr><td><code>{esc(f.id)}</code></td><td>{esc(f.titre)}</td>"
            f'<td class="prio-{f.priority}">{f.priority}</td>'
            f"<td>{esc(f.effort)}</td><td>{esc(f.categorie)}</td><td>{dep}</td></tr>"
        )
    if not rows:
...
def chantier_block(ch: Chantier) -> str:
    docs = "".join(f'<a href="{esc(d)}">📄 {esc(d)}</a>' for d in ch.docs)
    body = ""
    if ch.phases:
        ...
        for pid, plabel in ch.phases:
            body += f"<h3>{esc(pid)} — {esc(plabel)}</h3>" + fiche_rows(by_phase.pop(pid, []))
        for pid, extra in sorted(by_phase.items()):
            body += f"<h3>{esc(pid)} (phase non déclarée)</h3>" + fiche_rows(extra)
    else:
        body = fiche_rows(ch.fiches)
    return (
...
def cmd_dashboard(project: str, backlog: Path) -> int:
    ...
    counters = "".join(
        f'<div class="counter"><b style="color:{STATE_COLORS[s]}">'
        f"{sum(1 for f in all_fiches if f.state == s)}</b>"
        f"<span>{STATE_LABELS[s]}</span></div>"
        for s in STATES
    )
    ...
    for state in STATES:
        ...
        if not flat and not chs:
            continue
        body += f"<section><h2>{STATE_LABELS[state]}</h2>"
        for ch in chs:
            body += chantier_block(ch)
        if flat:
            body += fiche_rows(flat)
        body += "</section>"
    ...
    for ch in (c for c in chantiers if c.state == "fait"):
        ...
        apath.write_text(
            page(f"Chantier {ch.label} — archive", ch.desc, chantier_block(ch)),
            encoding="utf-8",
        )
